...
@section('title', __('admin.edit_surge_rule'))
@section('page_title', __('admin.edit_surge_rule'))
...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.surge-pricing.update', $rule->id) }}">
        @csrf @method('PUT')
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button class="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition">{{ __('admin.update_rule') }}</button>
        </div>
    </form>
@endsection
