...
@section('title', 'Driver Dues')
@section('page_title', 'Driver Due Management')
...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('admin.drivers_with_due') }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-red-100 dark:border-red-900/40 shadow-sm p-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('admin.drivers_at_limit') }}</p>
    ...
    <div class="flex items-center justify-between mb-3">
        ...
        <div class="flex gap-2 text-sm">
            <a href="{{ route('admin.payments.driver-dues') }}" class="rounded-lg px-3 py-1.5 {{ ! $showAll ? 'bg-indigo-600 text-white' : 'border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400' }}">{{ __('admin.due_gt_0') }}</a>
            <a href="{{ route('admin.payments.driver-dues', ['filter' => 'all']) }}" class="rounded-lg px-3 py-1.5 {{ $showAll ? 'bg-indigo-600 text-white' : 'border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400' }}">{{ __('admin.all') }}</a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        @if ($drivers->count())
                ...
                <table class="rr-table min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/70">
                        @foreach ($drivers as $driver)
                            @php($pct = $dueLimit > 0 ? min(100, round($driver->due_amount / $dueLimit * 100)) : 0)
                            <tr class="rr-row" x-data="{ clearing: false }">
                                <td class="px-4 py-3">
                                    <a href="{{ route('admin.drivers.show', $driver->id) }}" class="font-medium text-gray-800 dark:text-gray-100 hover:text-indigo-600">{{ $driver->name }}</a>
                                    <p class="text-xs text-gray-400 dark:text-gray-500">{{ maskPhone($driver->phone) }}</p>
                                </td>
                                <td class="px-4 py-3 text-end font-semibold {{ $driver->due_amount > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400' }}">{{ number_format($driver->due_amount, 2) }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ $driver->last_due_at ? \Illuminate\Support\Carbon::parse($driver->last_due_at)->format('d M Y') : '—' }}</td>
                                <td class="px-4 py-3 text-end">
                                    <div class="inline-flex items-center gap-3">
                                        <a href="{{ route('admin.drivers.show', ['id' => $driver->id, 'tab' => 'wallet']) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 text-xs font-medium">{{ __('admin.history') }}</a>
                                        @if ($canPay && $driver->due_amount > 0)
                                    ...
                                    @if ($canPay && $driver->due_amount > 0)
                                        <form method="POST" action="{{ route('admin.drivers.clear-due', $driver->id) }}" x-show="clearing" x-cloak class="mt-2 flex items-center gap-1 justify-end">
                                            @csrf
            ...
            @endif
        @else
            <x-admin.empty-state message="{{ __('admin.no_drivers_with_due') }}" />
        @endif
    </div>
@endsection
