...
class DriverAuthController extends Controller
{
     ...
     *
     * Logs in the first active approved driver with no phone/OTP/password.
    ...
    public function demoLogin(Request $request)
    {
        ...
        if (! $driver) {
            return $this->error('No approved demo driver found.', 404);
        }
        ...
        $token = $driver
            ->createToken('demo-mobile', ['driver'])
            ->plainTextToken;
    ...
    public function register(Request $request)
    {
        ...
        $store = fn ($field) =>
            $request
                ->file($field)
                ->store('drivers/' . $driver->id, 'public');
        ...
        DB::transaction(function () use (
            ...
            DriverDocument::create([
                'driver_id' => $driver->id,
                'type' => 'nid',
                'front_image' => $store('nid_front'),
                'back_image' => $store('nid_back'),
            ]);

            DriverDocument::create([
                'driver_id' => $driver->id,
                'type' => 'driving_license',
                'front_image' => $store('license_front'),
            ...
            DriverDocument::create([
                ...
                'expiry_date' =>
                    $data['vehicle_registration_expiry'],
            ]);
            ...
            if ($request->hasFile('insurance_doc')) {
                DriverDocument::create([
                    ...
                    'front_image' => $store('insurance_doc'),
                    'expiry_date' =>
            ...
            DriverDocument::create([
                'driver_id' => $driver->id,
                'type' => 'vehicle_photo',
                'front_image' => $vehicleFront,
                'back_image' => $vehicleBack,
            ]);
    ...
    public function status(Request $request)
    {
        ...
        return $this->success([
            ...
            'rejection_reason' => $driver->rejection_reason,
            'documents' => $driver->documents->map(
                fn ($d) => [
                    'type' => $d->type,
                    'status' => $d->status,
                    'expiry_date' =>
                        $d->expiry_date?->toDateString(),
                ]
            ),
        ]);
    }
