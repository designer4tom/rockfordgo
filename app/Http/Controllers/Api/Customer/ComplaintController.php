...
class ComplaintController extends Controller
{
    ...
    {
        $disputes = Dispute::where('raised_by', 'user')
        ...
        $items = $disputes->getCollection()->map(fn ($d) => [
            ...
            'refund_issued' => (bool) $d->refund_issued,
    ...
    public function store(Request $request)
    {
        ...
        $owns = Order::where('id', $data['order_id'])->where('user_id', $request->user()->id)->exists();
        if (! $owns) {
        ...
        return $this->success(['complaint_id' => $dispute->id], 'Complaint submitted. We will review and respond within 24 hours.');
    }
}
