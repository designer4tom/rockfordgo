...
## Part 2 — Driver (rider) app: TODO (separate repo)
...
1. [ ] Grep for `delivery_charge` usage in the COD-collect / cash-collection UI; remove it from any "amount to collect" math.
2. [ ] Update the collect-cod response/error models for the new fields (if parsed).
3. [ ] Verify `cod_reminder.collect_amount` is shown without modification.
